...
// Function to split a line into tokens (assuming CSV format)
...
#pragma omp parallel
    {
        std::vector<CrashRecord> local_records;
...
#pragma omp for nowait
        for (int i = 0; i < lines.size(); ++i)
        {
            ...
            try
            {
                ...
                local_records.emplace_back(timestamp, place, people, vehicle, ColID);
            }
...
#pragma omp critical
        {
            records.insert(records.end(), local_records.begin(), local_records.end());
        }
    }
